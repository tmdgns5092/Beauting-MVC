...
function setTime(start, end) {
    ...
    start_time = parseInt(start[0]);
    end_time = parseInt(end[0]);
}
(function($) {
		...
		var defaults = {
			format_output: function(tim, mini, meri) {
			    if (settings.show_meridian) {
			        ...
			        tim = Math.min(Math.max(parseInt(tim), start_time), end_time);
			...
			custom_classes: '',
			min_hour_value: start_time,
			max_hour_value: end_time,
			show_meridian: false,
			step_size_hours: '1',
			step_size_minutes: '1',
